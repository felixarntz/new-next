...
export async function setupFoundation(opts: SetupOptions): Promise<void> {
  ...
  const tailwindFlag = opts.tailwind === true ? " --tailwind" : "";
  await exec(
    `${packageManager.createNextAppCommand} . --ts --app${tailwindFlag} ${packageManager.createNextAppFlag} --biome --yes${skipInitialInstall}`
  );
